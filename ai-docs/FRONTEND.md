...
| `/leaderboard` | Live team standings with tier filtering | Yes | No | `app/leaderboard/page.tsx` | `teams`, `profiles`, `tier_settings` |
...
| `/rules` | Activity scoring rules display | Yes | No | `app/rules/page.tsx` | `activity_rules` |
...
| `/admin` | Redirects to `/admin/scoring` | Yes | No* | `app/admin/page.tsx` | — |
...
| `/admin/announcements` | Send notices via Slack/Email | Yes | Yes** | `app/admin/announcements/page.tsx` | — |
...
\* Admin layout renders for any authenticated user but mutations require admin.
\** Admin required for mutations only; pages render for any authenticated user.
...
│   │   ├── SpartanNavLinks (Leaderboard, Submit, Teams, Profile, Rules)
...
│       └── SpartanNavLinks (mobile variant)
├── Toaster (sonner)
...
- Nav links: Leaderboard, Submit, Teams, Profile, Rules

### Mobile (<md)
- Compact top header with logo, admin icon, account menu
- Fixed bottom tab bar: Leaderboard, Submit, Teams, Profile, Rules
...
| `app-shell.tsx` | Server | Main layout wrapper with headers and navigation |
| `auth-button.tsx` | Server | Shows account menu or login link |
| `auth-refresh.tsx` | Client | Refreshes auth state on route changes |
...
| `rules-modal.tsx` | Client | Modal showing scoring rules |
| `sign-up-form.tsx` | Client | Registration form |
| `spartan-nav-links.tsx` | Server | Navigation links for main and mobile nav |
...
6. If admin-only, use `requireAdmin()` at the top
